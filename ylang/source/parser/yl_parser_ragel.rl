...
%%{
    ...
    action nl { diagnostics->newline( (int)( offset + ( p - buffer ) - 1 ) ); }
    ...
    action neof
    {
        ...
        diagnostics->error( sloc, "unexpected end of file" );
        fbreak;
    }
    ...
    action nchar
    {
        ...
        int sloc = (int)( offset + ( p - buffer ) );
        if ( fc >= 0x20 && fc <= 0x7E )
            diagnostics->error( sloc, "unexpected character '%c'", fc );
        else
            diagnostics->error( sloc, "unexpected character '\\x%02X'", fc );
        if ( diagnostics->error_count() >= ERROR_LIMIT )
            goto error;
    }
...
bool yl_parser::parse( const char* path )
{
    ...
    // Open file.
    diagnostics->set_filename( path );
    FILE* file = fopen( path, "r" );
    if ( ! file )
    {
        diagnostics->error( 0, "unable to open file" );
        return false;
    }
    
    
    // Name of root function.
    root->function->funcname = root->alloc.strdup( path );
...
#define MTOKEN( ... ) \
    ...
    while ( ! iseof )
    {
        ...
        {
            data.shrink();
            diagnostics->error( offset, "error reading file" );
            goto error;
        }

        const unsigned char* p      = buffer;
        const unsigned char* pe     = buffer + read;
        const unsigned char* eof    = iseof ? pe : nullptr;
