...
yl_script* yl_parse( const char* path, size_t argc, const char* const* argv )
{
    ...
    ast->function->scope = new ( ast->alloc ) yl_ast_scope(
                    YL_SCOPE_SCRIPT, nullptr, ast->function, ast->function );
...
yl_parser::yl_parser( yl_diagnostics* diagnostics )
    :   diagnostics( diagnostics )
    ,   root( std::make_unique< yl_ast >() )
{
    region_scope rscope( root->alloc );

    // Set up root function and scope.
    root->function = alloc< yl_ast_func >( -1 );
    root->functions.push_back( root->function );
    root->function->scope = alloc< yl_ast_scope >(
                    YL_SCOPE_SCRIPT, nullptr, root->function, root->function );
    root->function->block = alloc< yl_stmt_block >( -1 );
    root->function->scope->block = root->function->block;
}
...
void yl_parser::parameter( const char* param )
{
    // Check for duplicated parameters.
    if ( root->function->scope->names.find( param )
                    != root->function->scope->names.end() )
    {
        assert( ! "duplicate parameter" );
    }
    ...
    root->function->scope->names.emplace( param, name );
    root->function->parameters.push_back( name );
}

void yl_parser::varargs()
{
...
void yl_parser::close_switch( yl_ast_scope* switch_scope )
{
    ...
    if ( scope->kind == YL_SCOPE_IMPLICIT )
    {
        ...
        diagnostics->error( switch_scope->node->sloc, "missing break" );
                ...
                yl_ast_node* name, yl_name_list* params, bool yield )
{
    ...
    if ( name->kind != YL_NAME_NAME )
    {
        diagnostics->error( sloc, "invalid prototype name" );
        return;
    }
                ...
                yl_name_list* params, bool yield, bool dot )
{
    ...
    if ( name )
    {
        ...
        func->funcname = root->alloc.strdup( funcname.c_str() );
    }
    else
    {
        if ( dot )
...
void yl_parser::statement( yl_ast_node* stmt )
{
    ...
    */
    ...
    {
        diagnostics->error( stmt->sloc, "case outside switch" );
    }
    ...
    if ( scope->kind == YL_SCOPE_SWITCH )
    {
        ...
        */
        ...
        else
        {
            ...
            {
                diagnostics->error( stmt->sloc, "expected case" );
...
yl_ast_scope* yl_parser::continue_target( int sloc )
{
    for ( yl_ast_scope* scope = get_scope();
                    scope != nullptr; scope = scope->outer )
    {
    ...
    diagnostics->error( sloc, "continue outside loop" );
    return get_scope();
}
...
yl_ast_scope* yl_parser::break_target( int sloc )
{
    ...
    diagnostics->error( sloc, "break outside loop or switch" );
    return get_scope();
}
...
yl_ast_node* yl_parser::lookup( int sloc, const char* identifier, bool outer )
{
    ...
    // Perform lookup.
    yl_ast_name* name = nullptr;
    for ( yl_ast_scope* scope = local; scope != nullptr; scope = scope->outer )
    {
...
yl_ast_node* yl_parser::lvalue( yl_ast_node* lvalue )
{
    ...
    default:
    {
        diagnostics->error( lvalue->sloc, "invalid lvalue" );
        break;
    }
    }
...
yl_ast_node* yl_parser::unpack( yl_ast_node* expr )
{
    ...
    default:
        diagnostics->error( expr->sloc, "invalid unpack expression" );
        break;
    }
...
void yl_parser::check_vararg( int sloc )
{
    ...
    {
        diagnostics->error( sloc, "varargs used outside vararg function" );
    }
}
...
void yl_parser::check_yield( int sloc )
{
    ...
    {
        diagnostics->error( sloc, "yield used outside coroutine" );
    }
}
...
yl_ast_name* yl_parser::declare( int sloc, const char* name )
{
    ...
    if ( i != scope->names.end() )
    {
        diagnostics->error( sloc, "'%s' is already declared", name );
    }
...
void yl_parser::declname( int sloc, yl_ast_node* name, yl_ast_node* decl )
{
    ...
    {
        assign->lvalue = declqual( sloc, name, object == nullptr );
    }
                        ...
                        yl_name_list* params, bool yield )
{
    ...
    if ( ! proto )
    {
        diagnostics->error( sloc, "function overrides non-prototype" );
        return outer;
    }
    
    if ( proto->parameters == nullptr && params == nullptr )
    {
    ...
    if ( proto->parameters == nullptr || params == nullptr )
    {
        ...
        diagnostics->error( sloc, "function does not match prototype" );
        return outer;
    }
...
yl_ast_node* yl_parser::delval( yl_ast_node* delval )
{
    ...
    default:
        diagnostics->error( delval->sloc, "expression is not deletable" );
        break;
    }
