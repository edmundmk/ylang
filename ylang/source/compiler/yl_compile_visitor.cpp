...
int yl_compile_visitor::visit( yl_stmt_if* node, int count )
{
    ...
    */
    ...
    pop( v );
    int test_jump = jump( node->sloc, YL_JMPF, v );
    ...
    if ( node->iffalse )
    {
        int endif_jump = jump( node->sloc, YL_JMP, 0 );
...
int yl_compile_visitor::visit( yl_stmt_switch* node, int count )
{
    ...
    */
    ...
    for ( size_t i = 0; i < node->body->stmts.size(); ++i )
    {
        ...
        pop( v );
        unsigned w = push();
        op( node->sloc, YL_EQ, w, u, v );
        pop( w );
        case_jumps.emplace( cstmt, jump( node->sloc, YL_JMPT, w ) );
    }
    
    pop( u );
    
    int default_jump = jump( node->sloc, YL_JMP, 0 );
...
int yl_compile_visitor::visit( yl_stmt_while* node, int count )
{
    ...
    */
    ...
    patch( jump( node->sloc, YL_JMP, 0 ), continue_label );
...
int yl_compile_visitor::visit( yl_stmt_do* node, int count )
{
    ...
    */
    ...
    patch( jump( node->sloc, YL_JMPT, v ), top_label );
...
int yl_compile_visitor::visit( yl_stmt_foreach* node, int count )
{
    ...
    */
    ...
    op( node->sloc, node->eachkey ? YL_ITERKEY : YL_ITER, i, l, 0 );
    
    int entry_jump = jump( node->sloc, YL_JMP, 0 );
    ...
    if ( node->declare )
    {
        ...
        {
        case 1:
            op( node->sloc, YL_NEXT1, rv.r, 0, 0 );
            break;
            
        case 2:
            op( node->sloc, YL_NEXT2, rv.r, rv.r + 1, 0 );
            break;
            
        default:
            op( node->sloc, YL_NEXT, rv.r, rv.count, 0 );
            break;
        }
    ...
    else
    {
        ...
        {
        case 1:
            op( node->sloc, YL_NEXT1, rv.r, 0, i );
            break;
            
        case 2:
            op( node->sloc, YL_NEXT2, rv.r, rv.r + 1, i );
            break;
            
        default:
            op( node->sloc, YL_NEXT, rv.r, rv.count, i );
            break;
        }
    ...
    patch( jump( node->sloc, YL_JMPITER, i ), continue_label );
    close_continue( node->scope, continue_label );
...
int yl_compile_visitor::visit( yl_stmt_for* node, int count )
{
    ...
    */
    ...
    if ( node->condition )
    {
        unsigned v = push( node->condition );
        pop( v );
        test_jump = jump( node->sloc, YL_JMPF, v );
    }
    ...
    patch( jump( node->sloc, YL_JMP, 0 ), top_label );
...
int yl_compile_visitor::visit( yl_stmt_try* node, int count )
{
    ...
    */
    ...
    if ( node->clist.size() )
    {
        finally_jumps.push_back( jump( node->sloc, YL_JMP, 0 ) );
        ...
        for ( size_t i = 0; i < node->clist.size(); ++i )
        {
            ...
            {
                unsigned e = push();
                op( cstmt->sloc, YL_EXCEPT, e, 0, 0 );
                unsigned p = push( cstmt->proto );
                pop( p );
                pop( e );
                unsigned r = push();
                op( cstmt->sloc, YL_IS, r, e, p );
                pop( r );
                catch_jump = jump( cstmt->sloc, YL_JMPF, r );
            }
...
int yl_compile_visitor::visit( yl_stmt_delete* node, int count )
{
    for ( size_t i = 0; i < node->delvals.size(); ++i )
    {
        ...
        if ( delval->kind == YL_EXPR_KEY )
        {
            ...
            pop( v );
            op( node->sloc, YL_DELKEY, 0, v, key( expr->key ) );
        }
        else if ( delval->kind == YL_EXPR_INKEY )
        {
            ...
            pop( u );
            pop( v );
            op( node->sloc, YL_DELINKEY, 0, v, u );
        }
    }
...
int yl_compile_visitor::visit( yl_stmt_return* node, int count )
{
    ...
    pop_list( lv );
    op( node->sloc, YL_RETURN, lv.r, lv.count, 0 );
    return 0;
}
...
int yl_compile_visitor::visit( yl_ast_func* node, int count )
{
    ...
    for ( size_t i = 0; i < func->upvals.size(); ++i )
    {
        ...
        case YL_UPVAL_LOCAL:
        {
            ...
            op( node->sloc, YL_UPLOCAL, (unsigned)i, ui, li );
            break;
        }
        ...
        case YL_UPVAL_UPVAL:
        {
            op( node->sloc, YL_UPUPVAL, (unsigned)i, uv.upval, 0 );
            break;
        }
        }
    }
...
{
    unsigned r = push();
    op( node->sloc, YL_BOOL, r, node->value ? 1 : 0 );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_NUMBER, r, number( node->value ) );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_STRING, r, string( node->string, node->length ) );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_MOV, r, local_index( node->name ), 0 );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_GLOBAL, r, 0, key( node->name ) );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_GETUP, r, (unsigned)node->index, 0 );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_MOV, r, local_index( node->object ), 0 );
    return 1;
}
...
int yl_compile_visitor::visit( yl_expr_key* node, int count )
{
    ...
    pop( o );
    unsigned r = push();
    op( node->sloc, YL_KEY, r, o, key( node->key ) );
    return 1;
}
...
int yl_compile_visitor::visit( yl_expr_preop* node, int count )
{
    ...
    op( node->sloc, YL_NUMBER, n, number( 1.0 ) );
    pop( n );
    pop( v );
    unsigned w = push();                        // lv, w
    op( node->sloc, node->opkind == YL_ASTOP_PREINC ? YL_ADD : YL_SUB, w, v, n );
    assign( lv, w );                            // *lv = w
    pop( w );
    pop_lvalue( lv );
    unsigned r = push();
    op( node->sloc, YL_MOV, r, w, 0 );           // w
    return 1;
}
...
int yl_compile_visitor::visit( yl_expr_postop* node, int count )
{
    ...
    op( node->sloc, YL_MOV, u, v, 0 );
    unsigned n = push();                        // lv, v, u, 1
    op( node->sloc, YL_NUMBER, n, number( 1.0 ) );
    pop( n );
    pop( u );
    unsigned w = push();                        // lv, v, w
    op( node->sloc, node->opkind == YL_ASTOP_POSTINC ? YL_ADD : YL_SUB, w, u, n );
    assign( lv, w );                            // *lv = w
    pop( w );
    pop( v );
    pop_lvalue( lv );
    unsigned r = push();
    op( node->sloc, YL_MOV, r, v, 0 );           // v
    return 1;
}
...
int yl_compile_visitor::visit( yl_expr_unary* node, int count )
{
    ...
    case YL_ASTOP_POSITIVE:
        op( node->sloc, YL_NEG, r, v, 0 );
        op( node->sloc, YL_NEG, r, r, 0 );
        break;
    ...
    case YL_ASTOP_LOGICNOT:
        op( node->sloc, YL_LNOT, r, v, 0 );
        break;

    case YL_ASTOP_BITNOT:
        op( node->sloc, YL_BITNOT, r, v, 0 );
        break;
...
int yl_compile_visitor::visit( yl_expr_binary* node, int count )
{
    ...
    case YL_ASTOP_MULTIPLY:
        op( node->sloc, YL_MUL, r, a, b );
        break;
        
    case YL_ASTOP_DIVIDE:
        op( node->sloc, YL_DIV, r, a, b );
        break;
        
    case YL_ASTOP_MODULUS:
        op( node->sloc, YL_MOD, r, a, b );
        break;
        
    case YL_ASTOP_INTDIV:
        op( node->sloc, YL_INTDIV, r, a, b );
        break;
        
    case YL_ASTOP_ADD:
        op( node->sloc, YL_ADD, r, a, b );
        break;
        
    case YL_ASTOP_SUBTRACT:
        op( node->sloc, YL_SUB, r, a, b );
        break;
        
    case YL_ASTOP_LSHIFT:
        op( node->sloc, YL_LSL, r, a, b );
        break;
        
    case YL_ASTOP_LRSHIFT:
        op( node->sloc, YL_LSR, r, a, b );
        break;
        
    case YL_ASTOP_ARSHIFT:
        op( node->sloc, YL_ASR, r, a, b );
        break;
        
    case YL_ASTOP_BITAND:
        op( node->sloc, YL_BITAND, r, a, b );
        break;
        
    case YL_ASTOP_BITXOR:
        op( node->sloc, YL_BITXOR, r, a, b );
        break;
        
    case YL_ASTOP_BITOR:
        op( node->sloc, YL_BITOR, r, a, b );
        break;
        
    case YL_ASTOP_CONCATENATE:
        op( node->sloc, YL_CONCAT, r, a, b );
        break;
...
int yl_compile_visitor::visit( yl_expr_compare* node, int count )
{
    ...
    for ( size_t i = 0; i < last; ++i )
    {
        ...
        pop( r );
        shortcut_jumps.push_back( jump( node->sloc, YL_JMPF, r ) );
        unsigned a = push();                    // a
        op( node->sloc, YL_MOV, a, b, 0 );
    }
...
int yl_compile_visitor::visit( yl_expr_logical* node, int count )
{
    ...
    case YL_ASTOP_LOGICAND:
    {
        ...
        pop( a );
        int shortcut_jump = jump( node->sloc, YL_JMPF, a );
        push( node->rhs );                      // b
    ...
    case YL_ASTOP_LOGICOR:
    {
        unsigned a = push( node->lhs );         // a
        pop( a );
        int shortcut_jump = jump( node->sloc, YL_JMPT, a );
        push( node->rhs );                      // b
        patch( shortcut_jump, label() );
        break;
    }
...
int yl_compile_visitor::visit( yl_new_new* node, int count )
{
    ...
    pop( t );
    pop( m );                                   // o
    op( node->sloc, YL_CALL, m, l.count != -1 ? l.count + 2 : -1, 0 );
    return 1;
}
...
{
    unsigned r = push();
    op( node->sloc, YL_ARRAY, r, (unsigned)node->values.size() );
    
    for ( size_t i = 0; i < node->values.size(); ++i )
    {
        unsigned v = push( node->values.at( i ) );
        pop( v );
        op( node->sloc, YL_APPEND, r, v, 0 );
    }
    ...
    if ( node->final )
    {
        ...
        pop_list( l );
        op( node->sloc, YL_EXTEND, r, l.r, l.count != -1 ? l.count : -1 );
    }
...
{
    unsigned r = push();
    op( node->sloc, YL_TABLE, r, (unsigned)node->elements.size() );
    ...
    for ( size_t i = 0; i < node->elements.size(); ++i )
    {
        ...
        pop( v );
        pop( k );
        op( node->sloc, YL_SETINDEX, r, k, v );
    }
...
int yl_compile_visitor::visit( yl_expr_call* node, int count )
{
    ...
    case YL_EXPR_INKEY:
    {
        ...
        f = push();
        t = push();
        op( node->sloc, YL_INKEY, t, o, k );     // o, m
        op( node->sloc, YL_SWP, f, t, 0 );       // m, o
        break;
    }
...
int yl_compile_visitor::visit( yl_expr_yield* node, int count )
{
    ...
    op( node->sloc, YL_YIELD, l.r, lcount, (unsigned)count );
...
int yl_compile_visitor::visit( yl_expr_vararg* node, int count )
{
    ...
    {
        listval r = push_list( count );
        op( node->sloc, YL_VARARG, r.r, r.count, 0 );
    }
    return count;
}
...
int yl_compile_visitor::visit( yl_expr_unpack* node, int count )
{
    if ( count != 0 )
    {
        ...
        pop( v );
        listval r = push_list( count );
        op( node->sloc, YL_UNPACK, v, r.r, r.count );
    }
    return count;
}
...
int yl_compile_visitor::visit( yl_expr_assign_list* node, int count )
{
    ...
    if ( node->assignop == YL_ASTOP_DECLARE )
    {
        ...
        else
        {
            ...
            op( node->sloc, YL_NULL, rv.r, rv.count, 0 );
        }
        ...
        for ( size_t i = 0; i < rcount; ++i )
        {
            op( node->sloc, YL_MOV, r.r + (unsigned)i, rv.r + (unsigned)i, 0 );
        }
    ...
    else
    {
        ...
        for ( size_t i = 0; i < rcount; ++i )
        {
            op( node->sloc, YL_MOV, r.r + (unsigned)i, v.at( i ), 0 );
        }
...
void yl_compile_visitor::add_break( int sloc, yl_ast_scope* target )
{
    ...
    branch_break.jumps.push_back( jump( sloc, YL_JMP, 0 ) );
}
...
void yl_compile_visitor::add_continue( int sloc, yl_ast_scope* target )
{
    ...
    branch_continue.jumps.push_back( jump( sloc, YL_JMP, 0 ) );
}
